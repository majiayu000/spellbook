...
These examples are designed for quick evaluation after installing Claude Arsenal. Pick one workflow, paste the prompt into Claude Code, and inspect the generated plan or files before letting the agent edit.
...
```bash
./install.sh --skills codebase-audit,fixflow,systematic-debugging
```
...
```bash
./install.sh --skills frontend-design,app-ui-design,ui-design-system
```
...
```bash
./install.sh --skills server-deploy,server-security,system-doctor
```
...
```bash
./install.sh --skills product-discovery,prd-master,technical-spec,product-analytics
```

Prompt:

```text
Use product-discovery and prd-master to turn this idea into a one-page PRD: a Claude Code skill dashboard that shows installed skills, stale descriptions, validation status, and suggested bundles. Include success metrics and out-of-scope items.
```
