...
5. At each `checkpoint_every_items` cadence, stop spawning when item, model-call, or time usage reaches its validated ceiling. If measured token usage materially exceeds the calibration projection, stop and request a revised budget. Close completed children and update the existing queue-ledger artifact listed in `output_firewall.evidence_paths`; do not invent a checkpoint run phase, and append the final run log only after collection.
