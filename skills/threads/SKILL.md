...
- Check review threads after PR creation/update, after CI completes, and immediately before merge. After marking a draft ready or after CI finishes, wait 60-120 seconds and re-check once before treating an empty `reviewThreads` result as clean or recording `no_connector_expected`. If a GitHub/Codex review connector was requested or is expected, do not merge until current-head connector completion is proven or `no_connector_expected` is recorded.
