...
pauseUI();
function commit(n){index=n;scene.src=slides[n].src;scene.alt=slides[n].alt;document.querySelector('#counter').textContent=`0${n+1} / ${String(slides.length).padStart(2,'0')}`;document.querySelector('#scene-name').textContent=`0${n+1} — ${slides[n].name}`;elapsed=0;hero.style.setProperty('--focus',`${slides[n].focus*100}%`);hero.dataset.scene=String(n);selectors.forEach((b,i)=>b.setAttribute('aria-pressed',String(i===n)));imageFor((n+1)%slides.length);}
