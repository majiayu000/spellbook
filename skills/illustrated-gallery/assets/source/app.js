...
const brandName=((brandLink&&brandLink.getAttribute('aria-label'))||'').replace(/\s*首页$/,'').trim()||((brandLink&&brandLink.childNodes[0]&&brandLink.childNodes[0].textContent)||'Studio').trim();
const brandSlug=(((brandLink&&brandLink.childNodes[0]&&brandLink.childNodes[0].textContent)||brandName).trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,''))||'studio';
document.querySelector('#brief-form').addEventListener('submit',e=>{e.preventDefault();const idea=document.querySelector('#brief').value.trim();if(!idea){document.querySelector('#brief').setCustomValidity('请写下你的想法。');document.querySelector('#brief').reportValidity();return;}const blob=new Blob([`# ${brandName} 创意简报\n\n${idea}\n`],{type:'text/markdown;charset=utf-8'});const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`${brandSlug}-brief.md`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});document.querySelector('#brief').addEventListener('input',e=>e.target.setCustomValidity(''));
